...
class VoyagerBaseController extends BaseVoyagerBaseController
{
    ...
    public function importAnalyses(Request  $request)
    {
        $uploadedFile = $request->file('file-input');
        if ($uploadedFile->getClientOriginalExtension() !== 'xlsx') {
            return back()->with(['message' => 'Выберите excel файл формата xlsx',
                'alert-type' => 'error',]);
        }

        \Excel::import(new AnalysisImport(), $uploadedFile);

        return redirect()->back()->with(['message' => 'Импорт успешно завершился',
            'alert-type' => 'success',]);
    }
